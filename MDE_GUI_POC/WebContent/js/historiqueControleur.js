				...
				[
						...
						function historiqueControleur($scope, $http,
							...
							if (sessionStorage.getItem("utilConn") === null) {
								...
								$http.get('/MDE_Rest/Api/connexion/motif')
										.success(function(response) {
											$scope.listeMotifs = response;
										});
								// Requête de la liste des motifs de connexions
								$http.get('/MDE_Rest/Api/usager').success(
										function(response) {
								...
								$scope.validerCriteres = function() {
									...
									var choixSite = ($scope.siteChoisi ? $scope.siteChoisi
											.replace(/\t/g, "").replace(/\n/g,
													"")
											: "");
									var choixMotif = ($scope.motifChoisi ? $scope.motifChoisi
											.replace(/\t/g, "").replace(/\n/g,
													"")
											: "");
									var choixUsageNom = ($scope.usagerSaisi ? $scope.usagerSaisi
											.split(" ")[1]
											: "");
									var choixUsagePrenom = ($scope.usagerSaisi ? $scope.usagerSaisi
											.split(" ")[0]
											: "");
								...
								/**
								 * Boutons "Effacer les critères" : Effacer les
								 * critères
								 */
